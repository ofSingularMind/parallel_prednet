...
class Prediction(keras.layers.Layer):
    ...
    def call(self, inputs):
        # last_frame = tf.zeros_like(inputs)[..., -self.num_frameChannels:]#self.parent.get_last_frame()
        # if last_frame is not None:
        #     inputs = keras.layers.Concatenate(axis=-1)([inputs, last_frame])
        # else:
        #     last_frame = tf.zeros_like(inputs)[..., -self.num_frameChannels:] #.to(inputs.device)
        #     inputs = keras.layers.Concatenate(axis=-1)([inputs, last_frame])
        out = inputs
