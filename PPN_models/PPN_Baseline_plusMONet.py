...
from monet_pytorch_GH_michedev.model import Monet
        ...
        if direction == "top_down":
            ...
            if self.training_args['pan_hierarchical']:
                ...
                if self.top_layer:
                    ...
                    self.states["TD_inp"] = self.upsample(inputs[1])
                    self.states["TD_inp"] = keras.layers.ZeroPadding2D(paddings)(self.states["TD_inp"])
                    R_inp = keras.layers.Concatenate()([self.states["E"], self.states["R"], self.states["TD_inp"], self.states["M_Inp"]])
                else:
                    self.states["TD_inp"] = self.upsample(inputs[1])
                    self.states["TD_inp"] = keras.layers.ZeroPadding2D(paddings)(self.states["TD_inp"])
...
class ParaPredNet(keras.Model):
    def __init__(self, training_args, im_height=540, im_width=960, *args, **kwargs):
        ...
        self.monet = Monet.from_config(model='monet-SSM', dataset=None, scene_max_objects= 4, dataset_width=64, dataset_height=64).to('cpu')
        self.monet.load_state_dict(torch.load('/home/evalexii/Documents/Thesis/code/parallel_prednet/monet_pytorch_GH_michedev/model_weights/laptop/monet_weights.pth', map_location=torch.device('cpu')))
        self.monet.eval()
        ...
        for l, c in enumerate(self.layer_output_channels):
            ...
            if l < self.num_layers - 1:
                ...
                temp_TD = None
            if l == 0:
                num_obj_data_channels = 4 * (3+1)
                temp_M = tf.random.uniform((self.batch_size, self.resolutions[l, 0], self.resolutions[l, 1], num_obj_data_channels), maxval=255, dtype=tf.float32)
            else:
                temp_M = None
            temp_out = self.predlayers[l]([temp_BU, temp_TD, l_temp_P, temp_M], paddings=self.paddings[l])
    ...
    def call(self, inputs):
        ...
        for t in range(self.nt):
            ...
            # Obtain MONet outputs
            monet_inputs = np.moveaxis(inputs[:, t, ...].detach().numpy(), -1, 1)
            outputs = self.monet(monet_inputs)
            # masks = outputs['mask']
            # recon = outputs['slot']
            masks = outputs['mask'].detach().numpy()
            recon = np.moveaxis(outputs['slot'].detach().numpy(), 0, -1)
            masked_recon = np.expand_dims(masks, -1) * recon
            M_inp = np.concatenate([masks, masked_recon], axis=-1)
            ...
            for _ in range(self.num_passes):
                ...
                for l, layer in reversed(list(enumerate(self.predlayers))):
                    ...
                    # Bottom layer
                        ...
                        layer([BU_inp, TD_inp, P_inp, M_inp], direction="top_down", paddings=self.paddings[l])
