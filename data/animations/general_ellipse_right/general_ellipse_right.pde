...
float e_w, e_h; // width and height of the ellipse
...
int num_frames;
PImage[] images;
float d, e_w_divisor; // Random size value
...
// String save_dir;
// if (train_mode) {save_dir = "/home/evalexii/Documents/Thesis/code/parallel_prednet/data/animations/general_ellipse_vertical/frames/general_ellipse_vertical/";}
// else if (test_mode) {save_dir = "/home/evalexii/Documents/Thesis/code/parallel_prednet/data/animations/general_ellipse_vertical/frames/general_ellipse_vertical_test/";}
...
void setup() {
  ...
  e_h = height / 3; // Initial length of the ellipse
  speed = height / 8; // Initial speed of vertical movement
  randomizationRate = (int) (height / speed);
  posX = width / 2; // Initial X position
  posY = 0;//height / 2; // Initial Y position
...
void draw() {
  ...
  if (rand_size && exec_randomize) {
    ...
    e_w_divisor = random(1.5, 3);
    if (train_mode == true) {
      while (e_w_divisor >= 2 && e_w_divisor <= 2.25) {e_w_divisor = random(1.5, 3);}
    } else if (test_mode == true) {
      while (e_w_divisor < 2 || e_w_divisor > 2.25) {e_w_divisor = random(2, 2.25);}
    } else {
      println("Error: train_mode and test_mode not defined."); exit();
    }
  ...
  // Update position
  posY += speed;
  if (posY - e_h / 2 > height) { // Reset position when it goes beyond the screen
    posY = -e_h / 2;
    exec_randomize = true;
  }
