...
for epoch in range(num_epochs):
    ...
    for batch_idx, images in enumerate(dataloader):
        ...
        if steps == save_interval:  # Print every 10 mini-batches
            ...
            with open(WEIGHTS_PATH + 'log.txt', 'a+') as f:
                f.write(f'Epoch [{epoch + 1}/{num_epochs}], Step [{batch_idx + 1}/{len(dataloader)}], Loss: [{avg_loss:.4f}], Neg Log P(x): [{outputs["neg_log_p_x"]:.4f}], KL Mask: [{outputs["kl_mask"]:.4f}], KL Latent: [{outputs["kl_latent"]:.4f}], 100K*MSE Loss: [{outputs["mse_loss"]:.4f}]')
