...
class SequenceLatentMaintainer(layers.Layer):
    def __init__(self, training_args, n, k, m, latent_dim, num_classes, num_slv_keep, **kwargs):
        ...
        self.dense_layers = [layers.Dense(units, activation='relu', name=f"dense_{i}") for i, units in enumerate(self.dense_units)]
        # self.output_layer = layers.Dense(n*self.num_slv_keep)  # Output layer to produce (num_slv_keep, n) matrix
        self.logits_layer = layers.Dense(1, name="logits_dense")  # Output layer to produce (n,1) logits, one for each input vector
        ...
        self.historic_sequence_latent_vectors_stored = [
            self.add_weight(name=f"historic_SLVs_for_class_{i}", shape=(self.num_slv_keep, latent_dim), initializer='zeros', trainable=False)
            for i in range(self.num_classes)
        ]

        # Log-scale temperature to enforce positivity
        # self.log_temperature = self.add_weight(name="log_temperature", dtype=tf.float32, shape=(), initializer='zeros', trainable=True)
        # self.beta = 1e-6  # Small constant to prevent division by zero
        # self.init_states()

    def call(self, new_vectors, class_label):
        # Compute positive temperature
        # softmax_temperature = tf.exp(self.log_temperature) + self.beta
        ...
        # output = self.output_layer(x) # Shape: (num_slv_keep * n)
        # output = tf.reshape(output, (self.num_slv_keep, self.n))  # Reshape to (num_slv_keep, n)
        # # Apply softmax to get the soft selection matrix
        # softmax_output = tf.nn.softmax(output * 1e-6, axis=-1) # Shape: (num_slv_keep, n)
        ...
        logits = self.logits_layer(x) # Shape: (n, 1)
        ...
        # padded_vectors = self.pad_to_num_slv_keep(selected_vectors)
        self.historic_sequence_latent_vectors_stored[class_label].assign(selected_vectors)

        # Calulate the maintained_vectors_loss as inverse volume spanned 
        # maintained_vectors_loss = self.calculate_volume_spanned_loss(selected_vectors)
        maintained_vectors_loss = self.combined_loss(selected_vectors)

        # tiled_vectors = tf.tile(tf.expand_dims(selected_vectors, axis=0), [self.batch_size, 1, 1])  # Shape: (BS, num_slv_keep, latent_dim)
    ...
    def combined_loss(self, vectors, epsilon=1e-6):
        # Volume loss
        gram_matrix = tf.matmul(vectors, vectors, transpose_b=True)
        gram_matrix += tf.eye(tf.shape(gram_matrix)[0]) * epsilon
    ...
    def init_states(self):
        self.historic_sequence_latent_vectors = [None for _ in range(self.num_classes)]
        for i in range(self.num_classes):
            self.historic_sequence_latent_vectors[i] = tf.convert_to_tensor(self.historic_sequence_latent_vectors_stored[i])
...
class ObjectRepresentations(keras.Model):
    ...
    def call(self, frame):
        ...
        for i in range(self.num_classes):
            ...
            consolidated_vectors, maintained_vectors_loss = self.seq_latent_maintainer(new_class_sequence_latent_vectors, i) # Shape: (num_slv_keep, latent_dim)
            meta_latent_vectors = self.mlVAE.encode([consolidated_vectors, labels])  # (BS, meta_latent_dim), single meta_latent_vector tiled to match batch size
            # meta_latent_vector = self.create_meta_latent_vector(consolidated_vectors) # (BS, meta_latent_dim)
            object_rep = self.mlVAE.decode_object_rep((meta_latent_vectors, labels, class_sequence))  # (BS, H, W, 1)
            output_object_representations.append(object_rep)

        return tf.concat(output_object_representations, axis=-1), maintained_vectors_loss # (BS, H, W, num_classes * 1), (BS,)

    def create_meta_latent_vector(self, sequence_latent_vectors):
        # Assume sequence_latent_vectors is a tensor of shape (BS, num_slv_keep, latent_dim)
        flattened_sequence_latent_vectors = K.batch_flatten(sequence_latent_vectors)
        return self.meta_latent_former(flattened_sequence_latent_vectors)

    def remove_empty_frames(self, separated_images):
        # Stack the images along a new axis, if necessary
        stacked_images = tf.stack(separated_images)

        # Compute the sum across the desired axes
        sum_images = tf.reduce_sum(stacked_images, axis=[1, 2, 3, 4])
        
        # Alternatively, you can use tf.boolean_mask directly
        filtered_images = tf.boolean_mask(separated_images, sum_images > 0)

        return tf.unstack(filtered_images)
    ...
    def pretrain_meta_latent_vae(self, data):
        ...
        for i in range(self.nt):
            ...
            for c in range(self.num_classes):
                ...
                sequence_latent_vectors, recon_sequence_latent_vectors, z_mean, z_log_var = self.mlVAE([new_class_sequence_latent_vectors, labels])
                total_loss_part, seq_lv_recon_loss_part, kl_loss_part = self.mlVAE.compute_loss(sequence_latent_vectors, recon_sequence_latent_vectors, z_mean, z_log_var)
        ...
        final_loss = all_seq_lv_loss + 0.25*all_kl_loss

        self.clear_states()
        
        return final_loss, all_seq_lv_loss, 0.25*all_kl_loss
    ...
    def train_step(self, data):
        ...
        # Pre-training the meta-latent VAE
            ...
            with tf.GradientTape() as tape:
                total_loss, seq_lv_loss, kl_loss = self.pretrain_meta_latent_vae(data)
                grads = tape.gradient(total_loss, self.mlVAE.trainable_weights)
                self.optimizer.apply_gradients(zip(grads, self.mlVAE.trainable_weights))
            return {"loss": total_loss, "L_seq_lv": seq_lv_loss, "KL": kl_loss}
...
# 1. Pre-train the Sequence Latent Maintainer
# 2. Pre-train the Meta-Latent VAE
...
ml_VAE_weights_file = f"/home/evalexii/Documents/Thesis/code/parallel_prednet/model_weights/SSM/multiShape/OCPN_wOR_OR_weights_final.h5"

### SET PRETRAINING STEP HERE ###
training_args["pretraining_step"] = 1
if training_args["pretraining_step"] == 2: training_args["batch_size"] = 20
weights_files = [seq_VAE_weights_file, seq_maintainer_weights_file, ml_VAE_weights_file]
...
if training_args["pretraining_step"] == 0:
    ...
    OR_generator.mlVAE.trainable = False
...
# Load model weights
try: 
    OR_generator.load_weights(load_weights_file) 
...
if training_args["pretraining_step"] == 0:
    ...
    OR_generator.sVAE.trainable = False
    OR_generator.seq_latent_maintainer.trainable = False
    OR_generator.mlVAE.trainable = True
OR_generator.compile(optimizer='adam')
...
# Train sequence VAE
OR_generator.fit(train_dataset, epochs=epochs, validation_data=val_dataset, steps_per_epoch=250, validation_steps=50, callbacks=callbacks)
